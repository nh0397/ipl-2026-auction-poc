...
import { 
  ...
  Loader2, Users, Zap, ArrowUp, Pause, ChevronRight, Hand 
} from "lucide-react";
...
const POOL_ORDER = ["Marquee", "Pool 1", "Pool 2", "Pool 3"];
...
export default function AuctionPage() {
  ...
  const startAuction = async () => {
    setActionLoading(true);
    await supabase.from("auction_config").update({ status: "live", current_pool: startPool, updated_at: new Date().toISOString() }).eq("id", auctionConfig.id);
  ...
  const switchPool = async (pool: string) => {
    setActionLoading(true);
    await supabase.from("auction_config").update({ current_pool: pool, updated_at: new Date().toISOString() }).eq("id", auctionConfig.id);
    await pickNextPlayer(pool);
    setActionLoading(false);
  };

  const pickNextPlayer = async (pool: string) => {
    // Get random unblocked player from pool
    ...
    if (!pending || pending.length === 0) {
      ...
      if (currentIndex < POOL_ORDER.length - 1) {
        ...
        await supabase.from("auction_config").update({ current_pool: nextPool, updated_at: new Date().toISOString() }).eq("id", auctionConfig.id);
        await pickNextPlayer(nextPool);
      } else {
        // All pools done
        await supabase.from("auction_config").update({ status: "completed", updated_at: new Date().toISOString() }).eq("id", auctionConfig.id);
        await supabase.from("auction_state").update({ status: "waiting", current_player_id: null, current_bid: 0, current_bidder_id: null, current_bidder_name: null }).eq("id", auctionState.id);
        await fetchAll();
      }
      return;
    }
    ...
    // Random selection
    const randomPlayer = pending[Math.floor(Math.random() * pending.length)];
    const config = POOL_CONFIG[pool] || POOL_CONFIG["Pool 3"];

    // Mark player as on_block
    await supabase.from("players").update({ auction_status: "on_block" }).eq("id", randomPlayer.id);
    ...
    await supabase.from("auction_state").update({
      ...
      current_bid: config.basePrice,
      current_bidder_id: null,
      current_bidder_name: null,
      min_increment: config.minIncrement,
      passed_user_ids: [],
  ...
  const markSold = async () => {
    ...
    await supabase.from("players").update({
      status: "Sold",
      auction_status: "sold",
      sold_to: auctionState.current_bidder_name,
      sold_price: `${auctionState.current_bid} Cr`,
    }).eq("id", currentPlayer.id);

    // Deduct budget from buyer
    const { data: buyer } = await supabase.from("profiles").select("budget").eq("id", auctionState.current_bidder_id).single();
    if (buyer) {
      await supabase.from("profiles").update({
        budget: buyer.budget - auctionState.current_bid,
      }).eq("id", auctionState.current_bidder_id);
    }

    // Reset auction state
    await supabase.from("auction_state").update({ status: "sold", updated_at: new Date().toISOString() }).eq("id", auctionState.id);
  ...
  const markUnsold = async () => {
    ...
    await supabase.from("auction_state").update({ status: "unsold", updated_at: new Date().toISOString() }).eq("id", auctionState.id);
  ...
  const nextPlayer = async () => {
    setActionLoading(true);
    await pickNextPlayer(auctionConfig.current_pool);
    setActionLoading(false);
  };
  ...
  const placeBid = async () => {
    ...
    await supabase.from("auction_state").update({
      ...
      current_bidder_name: profile.full_name || profile.team_name || "Unknown",
      updated_at: new Date().toISOString(),
    }).eq("id", auctionState.id);
  ...
  const passOnPlayer = async () => {
    if (!profile || !auctionState) return;
    setActionLoading(true);

    const currentPassed: string[] = auctionState.passed_user_ids || [];
